...
export const mockPosts: SummaryPostListSchema = Array.from({ length: 5 }).map(() => ({
  id: 1,
  summary: "Dolore est ad nostrud id dolor. Sit ea consectetur labore et ea voluptate in do sit.",
  likesCount: 321,
  isLiked: true,
  picturedAt: moment(new Date()).format("LLLL"),
}));

export const mockPosts2: SummaryPostListSchema = Array.from({ length: 2 }).map(() => ({
  id: 1,
  summary:
...
export const mockPosts3: SummaryPostListSchema = Array.from({ length: 4 }).map(() => ({
  id: 1,
  summary:
